    ...
    public class UserLoginTask extends AsyncTask<String, Void, String[]> {
        ...
        protected String[] doInBackground(String... params) {
            try {
                ...
                JSONObject response = HttpConnectionHelper.SendRequest(urlConnection, objectToSend,null);
                if(response == null){
...
//                    ret.setPassword(response.getString("password"));
                    //retorno el applicationToken para usarlo en cada request hacia el server side
                    return new String[]{response.getString("authenticationToken"), response.getString("_id")};
        ...
        protected void onPostExecute(final String[] params) {
            //en 0 viene token en 1 el id
            mAuthTask = null;
            showProgress(false);
            TextView tv = (TextView) findViewById(R.id.txt_error_login);
            String token = params[0];
            String userId = params[1];
            if (token == null) {
                Log.d("Error:", "result en null.");
                tv.setText("Error, usuario inválido.");
                tv.setVisibility(View.VISIBLE);
            } else {
                SharedPreferences settings = getSharedPreferences(PREFS_NAME, 0);
                SharedPreferences.Editor editor = settings.edit();
                //TODO ver que guardar y si guardarlo aca o en otro lado
